...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # set up each existing user on the free plan
        for prof in orm['main.Profile'].objects.all():
            prof.plan = None
            prof.purchased_bytes = 0
            prof.usd_per_month = 0.0
            prof.band_count_limit = 10
            prof.customer_id = ''
            prof.save()

        # delete the free plan
        for plan in orm['main.AccountPlan'].objects.all():
            plan.delete()

        # add the premium plans
        lite_plan = orm['main.AccountPlan']()
        lite_plan.title = "Lite"
        lite_plan.usd_per_month = 12.00
        lite_plan.total_space = 1024 * 1024 * 1024 * 10 # 10 GB
        lite_plan.band_count_limit = 50
        lite_plan.save()

        pro_plan = orm['main.AccountPlan']()
        pro_plan.title = "Pro"
        pro_plan.usd_per_month = 24.00
        pro_plan.total_space = 1024 * 1024 * 1024 * 20 # 20 GB
        pro_plan.band_count_limit = 100
        pro_plan.save()

        pp_plan = orm['main.AccountPlan']()
        pp_plan.title = "Pro Plus"
        pp_plan.usd_per_month = 99.00
        pp_plan.total_space = 1024 * 1024 * 1024 * 100 # 100 GB
        pp_plan.band_count_limit = 10000
        pp_plan.save()

        # give each band the default amount of space
        for band in orm['main.Band'].objects.all():
            band.total_space = 1024 * 1024 * 1024 * 0.5
