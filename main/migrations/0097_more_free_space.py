...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for profile in orm['main.Profile'].objects.all():
            if profile.band_count_limit == 2:
                profile.band_count_limit = 10
                profile.save()

        for band in orm['main.Band'].objects.all():
            if band.total_space == 300 * 1024 * 1024:
                band.total_space = 500 * 1024 * 1024
                band.save()
    ...
    models = {
        ...
        'main.tempfile': {
            ...
            'death_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 8, 30, 14, 53, 27, 797974)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
