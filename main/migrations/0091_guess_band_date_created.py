...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for prof in orm['main.Profile'].objects.all():
            band = prof.solo_band
            band.date_created = prof.user.date_joined
            band.save()
            seen_bands[band.id] = band

        for band in orm['main.Band'].objects.all():
            if not seen_bands.has_key(band.id):
                # since we don't know, make something up.
                band.date_created = datetime.datetime.now()
                band.save()
    ...
    models = {
        ...
        'main.tempfile': {
            ...
            'death_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 8, 23, 5, 58, 51, 487416)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
