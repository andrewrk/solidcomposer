...
if(fragment.endsWith("\n")){writer.write("Jst.writeln(");fragment=fragment.substring(0,fragment.length-1);}else{writer.write("Jst.write(");}
...
return fragment.toString();},parse:function parse(src){src=src.replaceAll("&lt;","<");src=src.replaceAll("&gt;",">");var stack=new CharacterStack(src);var writer=new StringWriter();var c;var fragment;while(stack.hasMore()){if(Jst.isOpeningDelimiter(stack.peek())){c=stack.pop();if(stack.peek()=='%'){c=stack.pop();if(stack.peek()=="="){stack.pop();fragment=Jst.parseExpression(stack);Jst.appendExpressionFragment(writer,fragment);}else{fragment=Jst.parseScriptlet(stack);writer.write(fragment);}}else{stack.push(c);fragment=Jst.parseText(stack);Jst.appendTextFragment(writer,fragment);}}else{fragment=Jst.parseText(stack);Jst.appendTextFragment(writer,fragment);}}
return writer.toString();},write:function write(s){Jst.html+=s;},writeln:function writeln(s){Jst.write(s+"\n");},evaluate:function evaluate(src,args){with(args){Jst.html="";var script=Jst.parse(src);eval(script);return Jst.html;}},compile:function compile(src){return Jst.parse(src);},evaluateCompiled:function evaluateCompiled(script,args){with(args){Jst.html="";eval(script);return Jst.html;}},html:""};