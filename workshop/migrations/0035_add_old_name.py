...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for version in orm['workshop.ProjectVersion'].objects.all():
            if len(version.new_title) > 0:
                prev_version = orm['workshop.ProjectVersion'].objects.filter(project=version.project, pk__lt=version.id, song__isnull=False).order_by('-pk')[0]
                version.old_title = prev_version.song.title
                version.save()
